...
static const std::string _module_id("$Id: wallitem.cpp,v da628cac41fc 2011/01/13 14:15:46 michael $");
...
WallItem::WallItem( Wall *w, int wallnumber, QGraphicsScene *canvas )
  ...
  Vector from = ( offs + *(w->N1()) ) * factor + (wn==1?-1:1) * par.mu * 0.5 * factor * perp;
  Vector to = ( offs + *(w->N2()) ) *factor + (wn==1?-1:1) * par.mu * 0.5 * factor * perp;
...
void WallItem::setColor(void) {
  ...
  Wall *w=&getWall();
  double tr = wn==1?w->Transporters1(1)/(w->C1()->SumTransporters(1)):w->Transporters2(1)/(w->C2()->SumTransporters(1));
  CellBase *c = wn==1?w->C1():w->C2();
 
    if(par.i1==1)
    {
    if(wn==1 && w->Transporters1(1)>=par.i2  )
    {  diffcolor.setRgb( 0, 0, (int)( ( tr / (1 ) )*255.));}
    
    else if( wn==2 && w->Transporters2(1)>=par.i2   )
    {  diffcolor.setRgb( 0, 0, (int)( ( tr / (1 ) )*255.));}
    
    else if(wn==1 && w->Transporters1(1)<par.i2   )
    {  diffcolor.setNamedColor("transparent");}
     ...
     {  diffcolor.setNamedColor("transparent");}
       ...
       if (w->AuxinSource() && c->BoundaryPolP()) {
        setPen( QPen(purple ,  par.nu) );
    } else {
        if (w->AuxinSink() && c->BoundaryPolP()) {
            setPen( QPen(blue,  par.nu));
        } else {
           // if(w->Transporters1(1)>0.5)
            {setPen (QPen(diffcolor,  par.nu) );}
           // else
            {
          //      setPen (QPen(diffcolor2,  par.nu) );
            }
        }
    }
    }
...
{
    if (w->C1()->CellType()==1 || w->C2()->CellType()==1)
     { diffcolor2.setNamedColor("forestgreen");}
    else if(w->C1()->Stiffness()==1e+3 || w->C2()->Stiffness()==1e+3) { diffcolor2.setNamedColor("darkgrey");}
     else if (w->C1()->CellType()!=1 && w->C2()->CellType()!=1 && w->C1()->Stiffness()!=1e+3 && w->C2()->Stiffness()!=1e+3) {diffcolor2.setNamedColor("transparent");}
  if (w->AuxinSource() && c->BoundaryPolP()) {
    ...
    if (w->AuxinSink() && c->BoundaryPolP()) {
      setPen( QPen(blue, par.outlinewidth));
        ...
        if (w->C1()->CellType()==1 || w->C2()->CellType()==1){ setPen (QPen(diffcolor2, par.rho1) );}
        else if(w->C1()->Stiffness()==1e+3 || w->C2()->Stiffness()==1e+3){ setPen (QPen(diffcolor2, par.c) );}
        else { setPen (QPen(diffcolor2, par.c) );}
    }
  }
