...
void Mymodel::OnDivide(ParentInfo *parent_info, CellBase *daughter1, CellBase *daughter2) {
    ...
    daughter1->SetChemical(1,0);
    daughter2->SetChemical(1, 0);
    ...
    //else
    {
        ...
        //else
        {
                ...
                
            {
                ...
                c->EnlargeTargetArea(12*par->cell_expansion_rate);
                if(c->Area()>2*c->BaseArea())
                {c->Divide();}
                //c->DivideOverAxis(axis);
...
//    if(c->Index()==30||c->Index()==50||c->Index()==49||c->Index()==38||c->Index()==8||c->Index()==16||c->Index()==39||c->Index()==5) {c->SetStiffness(Z);}
    ...
    //if  (c->Index()==0||c->Index()==3||c->Index()==9||c->Index()==10||c->Index()==11||c->Index()==16||c->Index()==17||c->Index()==19)
     ...
     if(c->Index()==30||c->Index()==50||c->Index()==49||c->Index()==38||c->Index()==8||c->Index()==16||c->Index()==39||c->Index()==5)
...
void Mymodel::CellDynamics(CellBase *c, double *dchem) {
    ...
    if ( c->Index()==5||c->Index()==49 ) {dchem[0]=src-kd*A;}
...
/**    if(dchem[0]==src2 ){c->SetCellType(3);}**/
    ...
    //if (c->CellType()==1 && c->Chemical(1)>=600) {c->SetCellType(2);}
    //if (c->CellType()==2){c->SetStiffness(100);}
...
double Mymodel::PINflux(CellBase *this_cell, CellBase *adjacent_cell, Wall *w)
{
    ...
    //pin_flux=par->k1*this_cell->Chemical(1)*receptor_level/(par->km+this_cell->Chemical(1))-par->k2*w->Transporters1(1);
    
    //par->k1*this_cell->Chemical(1)-par->k2*w->Transporters1(1);
    ...
    if (!this_cell->BoundaryPolP()&&!adjacent_cell->BoundaryPolP())  //use
    {
        if (this_cell->Chemical(0)>=adjacent_cell->Chemical(0))  //use
        {
            ...
            //   w->setTransporters2(1,0);
            pin_flux=(1e-10)*((delta_auxin)/(delta_auxin+10))*this_cell->Chemical(1)-par->k2*w->Transporters1(1);
        ...
        else if (this_cell->Chemical(0)<adjacent_cell->Chemical(0)) //use
        {
            ...
            //     else if (w->getTransporter(this_cell,1)>0)
            {
                ...
                //          pin_flux=(1e-10)*((delta_auxin)/(1))*adjacent_cell->Chemical(1)/(par->km+adjacent_cell->Chemical(1))-par->k2*w->Transporters1(1);
