...
static const std::string _module_id("$Id: cellbase.cpp,v 8aaffb5565a2 2010/10/14 09:45:59 roeland $");
  ...
  Vector()
{
 ...
 //   centroid=0;
  ...
  intgrl_x=0.; intgrl_y=0.;
    s_intgrl_xx=0.; s_intgrl_xy=0.; s_intgrl_yy=0.;
    s_intgrl_x=0.; s_intgrl_y=0.;
  source = false;
  source_conc = 0.;
  source_chem = 0;
  at_boundary=false;
...
CellBase::CellBase(const CellBase &src) :  QObject(), Vector(src)
{
 ...
 //   centroid=src.centroid;
  ...
  intgrl_x=src.intgrl_x; intgrl_y=src.intgrl_y;
    ...
    s_intgrl_xx=src.s_intgrl_xx; intgrl_xy=src.s_intgrl_xy; intgrl_yy=src.s_intgrl_yy;
    s_intgrl_x=src.s_intgrl_x; intgrl_y=src.s_intgrl_y;
...
double CellBase::Shortaxis(Vector *shrtaxis, double *shortwidth)  const
{
    ...
    if (shrtaxis) {
        *shrtaxis = Vector(-ixy, lambda_b - ixx, 0);
        //   cerr << "ixx = " << ixx << ", ixy = " << ixy << ", iyy = " << iyy << ", area = " << area << endl;
    }
    
    if (shortwidth) {
        *shortwidth = 4*sqrt((rhs1-rhs2)/area);
    }
