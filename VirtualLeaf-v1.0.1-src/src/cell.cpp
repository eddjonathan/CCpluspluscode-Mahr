...
static const std::string _module_id("$Id: cell.cpp,v 15d600891648 2010/06/24 13:22:58 michael $");
...
public:
    double operator() ( Node &n)
    {
        ...
        rotmat.Rot2D(90);
        double loc_area_plus=0;
        double loc_area_minus=0;
        double diff_node2=0;
        
        double loc_area_plusb=0;
        double loc_area_minusb=0;
        double diff_node2b=0;
        
        double cell_index1=0;
        ...
        Vector diff_node2_norm;
        
        Node *i=&n;
        // for (vector<Node *>::iterator i=c.nodes.begin();i!=c.nodes.end();i++){}
        
        //for (list<Node *>::iterator i =  n.nodes.begin(); i!=n.nodes.end(); i++)
        {
            //  nodepos.x=(*i)->x;
            // nodepos.y=(*i)->y;
            
            for (list<Neighbor>::const_iterator j =  n.owners.begin(); j!=n.owners.end(); j++){
                Vector loc_area_plusb = rotmat*(*(j->nb2)-n);
                Vector loc_area_minusb = rotmat*(*(j->nb1)-n);
                
                Vector length_plus = (*j->nb2-n);
                Vector length_minus = (*j->nb1-n);
                
                length_plus_norm = length_plus.Norm();
                length_minus_norm = length_minus.Norm();

                
                if ((find_if (n.owners.begin(), n.owners.end(),
                              bind2nd(mem_fun_ref(&Neighbor::CellStiffnessEquals),1e+3))) != n.owners.end()||
                    n.boundary){diff_node2_norm+=0;}
                
              //  if(n.boundary){diff_node2_norm+=0;}
               // if(j->cell->BoundaryPolP()) {diff_node2_norm+=0;}
              
                else {  diff_node2_norm+=-(j->cell->Area()-j->cell->TargetArea())*(loc_area_plusb-loc_area_minusb)+
                     par.lambda_length*((length_plus_norm-50)*((length_plus)/(length_plus_norm))+ (length_minus_norm-50)*((length_minus)/(length_minus_norm)))/2;
...
#define MULTIPLE_NODES
  ...
  // later on.
    int n=(int)((dist/par.rho0)+0.5);  // PUT n=0 for no nodes insertion
...
void BoundaryPolygon::Draw(QGraphicsScene *c, QString tooltip)
{
  ...
  p->setPolygon(pa);
  p->setPen(par.outlinewidth>=0?QPen( QColor("black"),par.outlinewidth):QPen(Qt::NoPen));
...
void Cell::Draw(QGraphicsScene *c, QString tooltip)
{
  ...
  for (list<Node *>::const_iterator n=nodes.begin(); n!=nodes.end(); n++) {
    ...
    double sum_sigma_xx=0;
    double sum_sigma_yy=0;
    ...
    double totforce=0;
    double totstress=0;
    double totstress_comp=0;
    double totstress_normal=0;
    ...
    sigmaxx1=LoopNodesxxcentx(Mytestx(), Nodex());
    sigmaxx2=LoopNodesxxcentxplus1(Mytestx(), Nodex());
    sigmaxx3=LoopNodesxxcentxplus1crossx(Mytestx(), Nodex());
    sigmaxx4=LoopNodesxxcentxplus1crossf(Mytestx(), Nodex());
    
    sum_sigma_xx=((sigmaxx1+sigmaxx2)/(3*this->Area())+(sigmaxx3+sigmaxx4)/(6*this->Area()));
    
    sigmayy1=LoopNodesxxcenty(Mytesty(), Nodey());
    sigmayy2=LoopNodesxxcentyplus1(Mytesty(), Nodey());
    sigmayy3=LoopNodesxxcentyplus1crossy(Mytesty(), Nodey());
    sigmayy4=LoopNodesxxcentyplus1crossf(Mytesty(), Nodey());
    
    sum_sigma_yy=(sigmayy1+sigmayy2)/(3*this->Area())+(sigmayy3+sigmayy4)/(6*this->Area());
    
    sigmaxy1=LoopNodesxxcenty(Mytestx(), Nodey());
    sigmaxy2=LoopNodesxxcentx(Mytesty(), Nodex());
    sigmaxy3=LoopNodesxxcentyplus1(Mytestx(), Nodey());
    sigmaxy4=LoopNodesxxcentxplus1(Mytesty(), Nodex());
    sigmaxy5=LoopNodesxxcentyplus1crossy(Mytestx(), Nodey());
    sigmaxy6=LoopNodesxxcentxplus1crossx(Mytesty(), Nodex());
    sigmaxy7=LoopNodesxxcentyplus1crossf(Mytestx(), Nodey());
    sigmaxy8=LoopNodesxxcentxplus1crossf(Mytesty(), Nodex());
    
    sum_sigma_xy=(sigmaxy1+sigmaxy2+sigmaxy3+sigmaxy4)/(6*this->Area())+(sigmaxy5+sigmaxy6+sigmaxy7+sigmaxy8)/(12*this->Area());
    
  //  sum_sigma_xx=sum_sigma_xx/(4*(sum_sigma_xx*sum_sigma_yy-sum_sigma_xy*sum_sigma_xy));
  //  sum_sigma_yy=sum_sigma_yy/(4*(sum_sigma_xx*sum_sigma_yy-sum_sigma_xy*sum_sigma_xy));
  //  sum_sigma_xy=sum_sigma_xy/(4*(sum_sigma_xx*sum_sigma_yy-sum_sigma_xy*sum_sigma_xy));
    
    
    force_xx=LoopForcexx(Mytestx());
    force_yy=LoopForceyy(Mytesty());
    
     rhs1=(sum_sigma_xx+sum_sigma_yy)/2.;
     rhs2= sqrt((sum_sigma_xx-sum_sigma_yy)*(sum_sigma_xx-sum_sigma_yy)+4*sum_sigma_xy*sum_sigma_xy)/2.;
     lambda_b=rhs1+rhs2;
    lambda_c=rhs1;
    
    Vector stressaxis= Vector(-sum_sigma_xy, lambda_b-sum_sigma_xx,0);
    Vector stressaxis_c= Vector(sum_sigma_xy, lambda_c-sum_sigma_yy/2,0);
    
    Vector total_force=Vector(sum_sigma_xx, sum_sigma_yy,0);
    
   
    
    Vector stress_normalize=stressaxis.Normalised();
    
    double angle = short_axis.SignedAngle(stress_normalize);
  ...
  //  double width= 0;
    double stress_x=0;
    double stress_y=0;
    width=4*sqrt((rhs1-rhs2)/this->Area());
    stress_x=stressaxis.x;
    stress_y=stressaxis.y;

    totstress=par.d*(stressaxis.Norm());
  //  totstress_comp=par.e*((stressaxis.Norm())-(sum_sigma_yy));
 //   totstress_comp=par.e*stressaxis_c.Norm();
    
     totstress_comp=par.e*(pow(stress_x,1));
//    totstress_normal=par.e*(sum_sigma_xx+sum_sigma_yy)/2;
    
    
    totforce=par.r*pow(force_xx+force_yy, 1.0);
    //double totforce=0.00001*pow(total_force.SqrNorm(),0.5);
    
   // stressaxis.Normalise();
    
    
    
  //  double width= 4*sqrt((rhs1-rhs2)/this->Area());

    
    //  Vector from = nodepos-par.node_mag*force_vector;
    // Vector to = nodepos+par.node_mag*force_vector;
    
    

    QColor cell_color ;
    QColor cell_color2;
    //cell_color.setHsvF(totforce/(1+totforce),1,1);
    if (par.f==1){
        if(totstress>=0)
        {        cell_color2.setHsvF(totstress/(1+totstress),1,1);}
        else {cell_color2.setNamedColor("red");}
        
        }
    else if (par.f==2) {
    if(totstress_comp>=0)
    {cell_color2.setHsvF(totstress_comp/(1+totstress_comp),1,1);}
      else {cell_color2.setNamedColor("red");}
    ...
    if (par.i4==1){
        if(totforce>=0)
        {        cell_color2.setHsvF(totforce/(1+totforce),1,1);}
        else {cell_color2.setNamedColor("red");}
    }
 ...
 // p->setPen(par.outlinewidth>=0?QPen( QColor(cell_color),par.outlinewidth):QPen(Qt::NoPen));
    if (par.f==1 || par.f==2|| par.i4==1){p->setBrush( cell_color2 );}
    else {p->setBrush( cell_color );}
  p->setZValue(1);
...
/**
void Cell::DrawStressAxis(QGraphicsScene *c) const
{
    ...
    for (list<Node *>::const_iterator i=nodes.begin(); i!=nodes.end(); i++) {
        for (list<Neighbor>::const_iterator n=(*i)->owners.begin(); n!=(*i)->owners.end(); n++) {
        Vector loc_area_plus = rotmat*(*(n->nb2)-*(*i));
        Vector loc_area_minus = rotmat*(*(n->nb1)-*(*i));
        
        Vector length_plus = (*(n->nb2)-*(*i));
        Vector length_minus = (*n->nb1-*(*i));
        ...
        diff_node2_norm+= -(n->cell->Area()-n->cell->target_area)*(loc_area_plus_norm-loc_area_minus_norm);
        
        
        //if(i->cell->Stiffness()==2e+3)
     //   if ((find_if (owners.begin(), owners.end(),
       //               bind2nd(mem_fun_ref(&Neighbor::CellStiffnessEquals),2e+3))) != owners.end())
       // {diff_node2=0;}
       // else
            //if(i->cell->Stiffness()!=2e+3)
        {        diff_node2+= -(n->cell->Area()-n->cell->target_area)*(loc_area_plus-loc_area_minus)+
        ...
        if (!this->BoundaryPolP()){
            from = *this-par.node_mag*diff_node2;
            to = *this+par.node_mag*diff_node2;
        }
        
        else if (this->BoundaryPolP())
        {
            from = *this;
            to = *this;
        }
        ...
        if ((find_if ((*i)->owners.begin(), (*i)->owners.end(),
                      bind2nd(mem_fun_ref(&Neighbor::CellTypeEquals),1))) != (*i)->owners.end())
        {
            // if (*this->x-to.x)
            arrow->setPen( QPen(QColor("yellow"),0.7));
            
        }
        else { arrow->setPen( QPen(QColor("sky blue"),0.7));}
        arrow->setZValue(2);
...
void Cell::DrawAxis(QGraphicsScene *c) const {
    ...
    double width2;
    Length(&long_axis, &width);
    Length(&long_axis2, &width2);
    
    //cerr << "Length is "  << length << endl;
    long_axis.Normalise();
    Vector short_axis=long_axis.Perp2D();
    Vector short_axis2=long_axis2.Perp2D();
  ...
  //  if (long_axis2>=2*short_axis2){ m->plugin->SetCellColor(this,QColor("black") );}
}

void Cell::DrawStrain(QGraphicsScene *c)  {
// MyWarning::warning("Sorry, Cell::DrawStrain temporarily not implemented.");
  ...
  //  Mesh mesh;
    ...
    double rhs1=(sum_sigma_xx+sum_sigma_yy)/2.;
    double rhs2= sqrt((sum_sigma_xx-sum_sigma_yy)*(sum_sigma_xx-sum_sigma_yy)+4*sum_sigma_xy*sum_sigma_xy)/2.;
    ...
    double width= 4*sqrt((rhs1-rhs2)/this->Area());
    //Vector centroid = Centroid();
    Vector from = centroid - par.node_mag * 1 * stressaxis;
    Vector to = centroid + par.node_mag * 1 *stressaxis;
 ...
 if (!this->AtBoundaryP())
  ...
  //else
  {line->setPen( QPen( QColor("brown"),2) );
    line->setZValue(2);
...
void Cell::DrawFluxes(QGraphicsScene *c, double arrowsize)
{
  ...
  Vector to = centroid + vec_flux/2.;
    ...
    if (this->Stiffness()==1e+3){arrow->setPen( QPen(QColor(Qt::transparent),0.5));}
    else {arrow->setPen( QPen(QColor(Qt::black),0.5));}
 // arrow->setPen( QPen(QColor(Qt::black),0.5));
  arrow->setZValue(2);
