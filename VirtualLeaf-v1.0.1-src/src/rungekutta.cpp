...
static const std::string _module_id("$Id: rungekutta.cpp,v 15d600891648 2010/06/24 13:22:58 michael $");
   ...
   routine to be used. */
{
  ...
  *nok = (*nbad) = kount = 0;
  ...
  for (nstp=0;nstp<par.maxt;nstp++) { //Take at most Maxstp steps.
    derivs(x,y,dydx);
    for (i=0;i<nvar;i++)
