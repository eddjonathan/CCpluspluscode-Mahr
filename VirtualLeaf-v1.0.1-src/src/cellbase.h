/*
 *
 *  $Id: cellbase.h,v 8aaffb5565a2 2010/10/14 09:45:59 roeland $
 *
...
class CellBase :  public QObject, public Vector 
{
 ...
 public:
  ...
  double Shortaxis(Vector *shrtaxis = 0, double *shortwidth = 0) const;
    double StressLength(Vector *stress_long_axis = 0, double *stress_width = 0) const;
 ...
 protected:
  ...
  mutable double intgrl_xx, intgrl_xy, intgrl_yy, intgrl_x, intgrl_y;
     mutable double s_intgrl_xx, s_intgrl_xy, s_intgrl_yy, s_intgrl_x, s_intgrl_y;  //I CREATED THIS
...
inline Vector PINdir(CellBase *here, CellBase *nb, Wall *w)
{
  ...
  return   w->getInfluxVector(here)*(w->getTransporter( here, 1)) ;
}
