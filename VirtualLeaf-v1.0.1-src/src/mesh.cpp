...
static const std::string _module_id("$Id: mesh.cpp,v 2accd83924da 2010/11/25 13:57:33 roeland $");
	...
	w2 = 2;
	  ...
	  w2 = 1;
          **/
          ...
          if (node.boundary && cit->nb1->boundary )
          {
              //if (cit->cell->Stiffness()==5e+2)
              {
                  if((find_if(node.owners.begin(), node.owners.end(), bind2nd(mem_fun_ref(&Neighbor::CellStiffnessEquals),1)))!=node.owners.end())
                  {w1=par.rel_perimeter_stiffness;}
                  
                  //else if((find_if(node.owners.begin(), node.owners.end(), bind2nd(mem_fun_ref(+&Neighbor::CellStiffnessEquals),2e+3)))!=node.owners.end())
                  else w1=10;
              }
              /**else
               {
          ...
          else
          {
             // w1=2;
              ...
              //if( (find_if(node.owners.begin(), node.owners.end(), bind2nd(mem_fun_ref(&Neighbor::CellTypeEquals),1)))!=node.owners.end() && (find_if(node.owners.begin(), node.owners.end(), bind2nd(mem_fun_ref(&Neighbor::CellTypeEquals),0)))!=node.owners.end())
              if((find_if(node.owners.begin(), node.owners.end(), bind2nd(mem_fun_ref(&Neighbor::CellTypeEquals),1)))!=node.owners.end() )
              {
              if((find_if(node.owners.begin(), node.owners.end(), bind2nd(mem_fun_ref(&Neighbor::CellTypeEquals),2)))!=node.owners.end() )
              { w1 = par.i2;}
               else w1=par.i3;
              }
              else w1=2;
              /**if((find_if(node.owners.begin(), node.owners.end(), bind2nd(mem_fun_ref(&Neighbor::CellStiffnessEquals),5e+2)))!=node.owners.end() )
               {
               w1 = 8;
               }
               if( (find_if(node.owners.begin(), node.owners.end(), bind2nd(mem_fun_ref(&Neighbor::CellStiffnessEquals),5e+2)))!=node.owners.end() && (find_if(node.owners.begin(), node.owners.end(), bind2nd(mem_fun_ref(&Neighbor::CellStiffnessEquals),1)))!=node.owners.end())
               {
               w1=1;
               }**/
          }
...
void Mesh::DrawStressAxis2(QGraphicsScene *c)
{
...
void Mesh::DrawStressAxis(QGraphicsScene *c) const
{
        ...
        for (list<Neighbor>::const_iterator n=(*i)->owners.begin(); n!=(*i)->owners.end(); n++) {
          ...
          //  if(*i->cell->Stiffness()==2e+3)
               if ((find_if ((*i)->owners.begin(), (*i)->owners.end(),
                           bind2nd(mem_fun_ref(&Neighbor::CellStiffnessEquals),2e+3))) != (*i)->owners.end())
             {diff_node2=0;}
             else
            //if(i->cell->Stiffness()!=2e+3)
...
//        Node *i=*n;
        if (!(*i)->BoundaryP()){
            from = *(*i)-par.node_mag*diff_node2;
            to = *(*i)+par.node_mag*diff_node2;
        }
      ...
      //  QGraphicsArrowItem *arrow = new QGraphicsArrowItem(0,c);
        ...
        {
            // if (*this->x-to.x)
        line->setPen( QPen(QColor("yellow"),0.7));
            
        }
        else { line->setPen( QPen(QColor("sky blue"),0.7));}
        line->setZValue(2);
...
void Mesh::ReactDiffuse(double delta_t) {
  ...
  solver->odeint(ystart, nvar, getTime(), getTime() + delta_t, 
		 par.ode_accuracy, par.dt, 1e-20, &nok, &nbad);
...
string Mesh::getTimeHours(void) const {
  ...
  ostringstream tstr;
  tstr << time<<"time"<< hours << " h " << mins << " m " << secs << " s\t\t"<<   NCells()<<"Total cells";
  return tstr.str();
}
...
void Mesh::CSVExportCellDatanodes(QTextStream &csv_stream) const {
    
    Mesh mesh ;
    
    {
        
        for (vector<Node *>::const_iterator j=nodes.begin();
             j!=nodes.end();
             j++)
        {
            Node &node(**j);
            csv_stream <<node.Index()<<","<<node.x << ", "<< node.y ;
            for (list<Neighbor>::const_iterator cit=node.owners.begin(); cit!=node.owners.end(); cit++)
            {
                Vector centroid = cit->cell->Centroid();
                {
                    
                    if ( cit->cell->Index()==-1){csv_stream <<", "<<cit->nb2->Index();}
                    else {csv_stream <<", "<<cit->nb1->Index();}
             ...
             j++)
        {
            ...
            for (list<Neighbor>::const_iterator cit=node.owners.begin(); cit!=node.owners.end(); cit++)
            {
                Vector centroid = cit->cell->Centroid();
                {
                    ...
                    if ( cit->cell->Index()!=-1){csv_stream <<", "<< cit->cell->Index()+1<<","<<cit->cell->CellType();
                        
                        csv_stream << ", " << cit->cell->Chemical(0)/(cit->cell->Area());}
