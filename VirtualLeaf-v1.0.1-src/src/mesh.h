/*
 *
 *  $Id: mesh.h,v 2accd83924da 2010/11/25 13:57:33 roeland $
 *
...
class Mesh {
 ...
 public: 
  ...
  template<class Op> void LoopNodes(Op f) {
    ...
    {
      f(**i);
        QVector<qreal> stressxx;
        QVector<qreal> stressyy;
        QVector<qreal> stressxy;
    ...
    template< class Op1> double LoopNodesyy(Op1 f) {
    ...
    template< class Op1> double LoopNodesxy(Op1 f) {
        //for (vector<Cell *>::const_iterator w=cells.begin();
        //   w!=cells.end();
        // w++)
        double sum=0;
 ...
 **/
  ...
  void DrawNodes(QGraphicsScene *c) const;
    ...
    void DrawStressAxis(QGraphicsScene *c)  const;
    void DrawStressAxis2(QGraphicsScene *c);
  ...
  void SettoInitVals(void);
    ...
    void Stresscompute(void);  /// FOR STRESS COMPUTE
  ...
  void CSVExportCellData(QTextStream &csv_stream) const;
     void CSVExportCellDatanodes(QTextStream &csv_stream) const;
